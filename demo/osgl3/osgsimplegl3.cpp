...
using namespace std;
void showState(osg::State* s);
void showShader(osg::Program* p);
...
void configureShaders( osg::StateSet* stateSet )
{
    ...
    LoadShaderSource(vShader.get(), "shader/g.vert");
    osg::ref_ptr<osg::Shader> fShader = new osg::Shader(osg::Shader::FRAGMENT);
    LoadShaderSource(fShader.get(), "shader/g.frag");
#endif
...
//    showShader(dynamic_cast<osg::Program*>(root->getOrCreateStateSet()->getAttribute(osg::StateAttribute::PROGRAM, 0)));
...
void showState(osg::State* s)
{
    auto alias = s->getTexCoordAliasList();
    for(auto a : alias){
        cout <<"Location:" <<a._location;
        cout <<" gl name:" <<a._glName;
        cout <<" osg name:" <<a._osgName;
        cout <<" declaration:" <<a._declaration <<endl;
    }
}
void showShader(osg::Program* p)
{
    ...
    cout <<"shaders:" <<p->getNumShaders() <<endl;
    for(int ix = 0; ix < p->getNumShaders(); ++ix){
        cout <<p->getShader(ix)->getShaderSource() <<endl;
    }
}
