...
public:
KeyEventHandler() : osgGA::GUIEventHandler()
{}
KeyEventHandler(const KeyEventHandler& eh, const osg::CopyOp& copyop = osg::CopyOp::SHALLOW_COPY)
    : osgGA::GUIEventHandler(eh, copyop)
{}
bool handle (const osgGA::GUIEventAdapter &ea, osgGA::GUIActionAdapter &aa, 
    osg::Object *, osg::NodeVisitor *) override
{
    static int modifer = 0;
    using osgGA::GUIEventAdapter;
    if(ea.getEventType() == GUIEventAdapter::KEYDOWN){
        int pp = ea.getModKeyMask();
        if(pp != modifer){
            modifer = pp;
            cout <<"ALT DOWN: " <<boolalpha <<((pp&GUIEventAdapter::MODKEY_ALT) != 0) <<endl;
        }
    }
        ...
        if(pp != modifer){
            modifer = pp;
            cout <<"ALT UP: " <<boolalpha <<((pp&GUIEventAdapter::MODKEY_ALT) == 0) <<endl;
        }
    }
    return false;
}
...
main(int argc, char** argv)
{
    ...
    app.viewer.addEventHandler(new KeyEventHandler);
    ...
    if ( node.valid() )
    {
		...
		ObjectIDPicker::Function clk = [&](ObjectID id)
        {
            if (id > 0)
            {
                // Got a pick:
                auto anno = Registry::objectIndex()->get<AnnotationNode>(id);
				if(anno.valid()){
					cout <<"ANNO: " <<anno->getName() <<endl;
				}
            }
        };
...
void createCustomAnno(osgEarth::MapNode* root)
{
    ...
    IC(img);
    auto anno = new PlaceNode(GeoPoint(srs, -74.00, 40.71), "占领地标识符", pm, img);
    anno->setName("测试标识");
    layer->addChild(anno);
